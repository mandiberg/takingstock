...
USE_ALL = True # this is for outputting all images from a oneshot, forces ONE_SHOT
...
##################MICHAEL#####################
    ...
    #TKbook remove this for full set
...
def linear_test_df(df_sorted,df_segment,cluster_no, itter=None):
        ...
        if USE_PAINTED and os.path.exists(inpaint_file):
            ...
            if do_inpaint and not bailout:
                ...
                if inpaint_image is not None:
                    ...
                    cv2.imwrite(inpaint_file+"3_lama.jpg",lama)
                    cv2.imwrite(inpaint_file+"4_premerge.jpg",premerge)
    ...
    # sort df_sorted by index
    df_sorted = df_sorted.sort_index()

    #itter is a cap, to stop the process after a certain number of rounds
    print('linear_test_df writing images, total number of images possible is ', str(len(df_sorted)))
    ...
    for index, row in df_sorted.iterrows():
        ...
        try:
            ...
            ### start removing conditionals here
            cropped_image, is_inpaint = sort.crop_image(img, row['face_landmarks'], row['bbox'])
            if cropped_image is None: print("cropped_image is None, before inpaint")
            print("cropped_image shape before inpaint", cropped_image)
        ...
        # print(sort.counter_dict)
            ...
            print(" >>>>>>>>    something went wrong with linear_test_df", index)
            traceback.print_exc()
            print(str(e))
        print("metas_list")
        print(metas_list)
...
def main():
    ...
    def map_images(resultsjson, cluster_no=None):
        ...
        if not df.empty:
            ...
            #TKbook remove this to avoid segmenting the dataframe
