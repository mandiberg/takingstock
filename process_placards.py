...
def _get_skip_state(result):
    ...
    skip_object_signature = False
    if EXCLUDE_EXISTING_OBJECT_SIGNATURE_NONES:
        ios_row = session.query(ImagesObjectSignatures).filter(
            ImagesObjectSignatures.image_id == image_id,
            ImagesObjectSignatures.cluster_id.in_(EXCLUDE_EXISTING_OBJECT_SIGNATURE_NONES)
        ).first()
        if ios_row is not None:
            print(f"Image {image_id} - Skipping: ImagesObjectSignatures.cluster_id={ios_row.cluster_id} in EXCLUDE_EXISTING_OBJECT_SIGNATURE_NONES.")
            skip_object_signature = True

    skip_all = (
        skip_object_signature or
