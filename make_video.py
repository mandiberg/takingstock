...
io = DataIO(IS_SSD)
db = io.db
CSV_FOLDER = os.path.join(io.ROOTSSD, "body3D_segmentbig_useall256_CSVs")
print("CSV folder", CSV_FOLDER)
#CSV_FOLDER = ("/Users/tenchc/Desktop/Work/Mandiberg/project_00/body3D_segmentbig_useall256_CSVs")
...
#TENCH CODE
IS_TENCH = False
if os.environ.get("USER") or os.environ.get("USERNAME") == "tenchc":
    IS_TENCH = True
...
def linear_test_df(df_sorted,segment_count,cluster_no, itter=None):
    ...
    for index, row in df_sorted.iterrows():
        ...
        try:
            ...
            # print("sort.counter_dict[""]", sort.counter_dict["outfolder"])
            outpath = os.path.join(sort.counter_dict["outfolder"],imgfilename)
            print("io.ROOT", io.ROOT)

            print(io.ROOT,row['folder'],row['imagename'])
            site_name_folder = os.path.basename(row['folder'])
            open_path = os.path.join(io.ROOT,site_name_folder,row['imagename'])
            print("openpah", open_path, outpath)
