...
def do_yolo_detections(result, image, image_path, existing_detections, custom=False, phase_times=None):
    image_id = result.image_id
    imagename = result.imagename
    ...
    if phase_times is not None:
        phase_times[infer_key] += time.time() - infer_start
    ...
    if len(detect_results) == 0:
        ...
        save_no_dectections(session,image_id, NoDetectionTable=ThisNoDetectionTable)
        if phase_times is not None:
            phase_times['postprocess_save'] += time.time() - postprocess_start
        return detect_results
    else:
...
def do_detections(result, folder_index, phase_times=None):
    ...
    if phase_times is not None:
        phase_times['skip_state'] += time.time() - skip_state_start
    ...
    if phase_times is not None:
        phase_times['image_load'] += time.time() - image_load_start_time
    if image is None:
    ...
    if phase_times is not None:
        phase_times['backfill'] += time.time() - backfill_start_time
    ...
    if DO_COCO and not skip_coco_due_to_no_det:
        coco_detections = do_yolo_detections(result, image, image_path, existing_detections, custom=False, phase_times=phase_times)

    if DO_CUSTOM and not skip_custom_due_to_no_det:
        custom_detections = do_yolo_detections(result, image, image_path, existing_detections, custom=True, phase_times=phase_times)
    ...
    for i in range(0, len(img_list), BATCH_SIZE):
        ...
        for site_image_id in batch_site_image_ids:
            ...
            if site_image_id in results_dict:
                do_detections(results_dict[site_image_id], folder_index, phase_times=interval_phase_times)
                processed_count += 1
                if processed_count % TIMER_INTERVAL == 0:
