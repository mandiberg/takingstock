...
def build_temp_tables(conn, helper_table):
...
def write_outputs(output_dir, helper_table, rates, slot_counts, global_slots):
    os.makedirs(output_dir, exist_ok=True)

    slot_matrix = (
        rates[rates["conf_band"] == "all_conf"]
        [["class_id"] + SLOTS + ["total_slot_assignments", "total_images"]]
    ...
    slot_counts.sort_values(["conf_band", "class_id", "slot"]).to_csv(
    ...
    global_slots.sort_values(["conf_band", "slot"]).to_csv(
...
def run_one_stage(engine, helper_table, output_root):
    ...
    write_outputs(output_dir, helper_table, rates, slot_counts, global_slots)
...
def parse_args():
    ...
    parser.add_argument(
        "--helper-table",
        type=str,
        default=HELPER_TABLE_PROGRESSION[0],
...
def main():
    ...
    if args.run_progression:
        for helper in HELPER_TABLE_PROGRESSION:
            run_one_stage(engine, helper, args.output_root)
    else:
        run_one_stage(engine, args.helper_table, args.output_root)
