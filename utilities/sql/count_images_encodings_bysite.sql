...
-- matrix of fusion poses BODIES
-- GOOD GOOD GOOD April 22 2025
-- generates a 32 x 128 grid of BODIES x gesture cluster counts   
   
SELECT 
    ibp.cluster_id AS ibp_cluster,  -- Row: ImagesHandsPoses cluster_id
    SUM(CASE WHEN ihg.cluster_id = 0 THEN 1 ELSE 0 END) AS ihg_1,
    SUM(CASE WHEN ihg.cluster_id = 1 THEN 1 ELSE 0 END) AS ihg_1,
    SUM(CASE WHEN ihg.cluster_id = 2 THEN 1 ELSE 0 END) AS ihg_2,
    SUM(CASE WHEN ihg.cluster_id = 3 THEN 1 ELSE 0 END) AS ihg_3,
...
SUM(CASE WHEN ihg.cluster_id =  127 THEN 1 ELSE 0 END) AS ihg_127 
    -- Add more CASE statements for additional clusters if necessary
    -- SUM(CASE WHEN ih.cluster_id = 75 THEN 1 ELSE 0 END) AS ih_75 -- Example cluster for ibp
FROM 
    SegmentOct20 so
JOIN 
    ImagesBodyPoses ibp ON ibp.image_id = so.image_id
JOIN 
    ImagesHandsGestures ihg ON ihg.image_id = so.image_id
JOIN ImagesTopics it ON it.image_id = so.image_id
WHERE so.is_feet = 1
-- WHERE it.topic_id = 32
GROUP BY 
    ibp.cluster_id  -- Group by ImagesHandsPoses cluster_id to create rows
ORDER BY 
    ibp_cluster;
...
--- select count by topic for a given pose cluster fusion

SELECT it.topic_id, COUNT(so.image_id) as ccount
