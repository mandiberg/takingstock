...
USE STock;
SELECT DISTINCT(s.image_id), s.site_name_id, s.contentUrl, s.imagename, s.description, s.face_x, s.face_y, s.face_z, s.mouth_gap, s.bbox, s.site_image_id, e.pitch, e.yaw, e.roll, ibg.lum, ibg.lum_bb, ibg.hue, ibg.hue_bb, ibg.sat, ibg.sat_bb, ibg.val, ibg.val_bb, ibg.lum_torso, 
ibg.lum_torso_bb , d.bbox_norm, d.obj_no, d.orientation, d.meta_cluster_id 
;

SELECT COUNT(DISTINCT s.image_id )
FROM SegmentBig_isface s  JOIN Encodings e ON s.image_id = e.image_id  
JOIN ImagesArmsPoses3D ihp ON s.image_id = ihp.image_id  
-- JOIN ImagesKeywords it ON s.image_id = it.image_id  
JOIN SegmentHelperObject_83_bag sh ON s.image_id = sh.image_id  
-- JOIN ImagesBackground ibg ON s.image_id = ibg.image_id  
JOIN Detections d ON s.image_id = d.image_id   
...
AND ihp.cluster_id = 48 
AND d.class_id  IN (88)  LIMIT 200000;


SELECT COUNT(*)
FROM  Detections it
WHERE it.class_id = 88
AND JSON_VALUE(it.bbox_norm, '$.left') IS  NULL;

;


SELECT ihp.cluster_id, COUNT(*)
FROM  Detections it
JOIN ImagesArmsPoses3D ihp ON it.image_id = ihp.image_id 
WHERE it.class_id = 88
GROUP BY ihp.cluster_id 
ORDER BY ihp.cluster_id 
;
